    ...
    public partial class IncomeForm : UserControl
    {
        ...
        private void IncomeAddButton_Click(object sender, EventArgs e)
        {
            ...
            } else
            {
                ...
                {
                    connect.Open();
                    string query = "INSERT INTO income (category, item, income, description, date_income, date_insert)" +
                        "VALUES (@cat, @item, @income, @desc, @date_i, @date)";
            ...
            else
            {
                using (SqlConnection connect = new SqlConnection(stringConnect))
                {
                    ...
                    string query = "UPDATE income SET category=@cat, item=@item, income=@income, " +
                                   "description=@desc, date_income=@date_i WHERE id=@id";
